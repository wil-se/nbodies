...
void compute_ex_force(int body2, int body1){
	...
	long double force[3] = {0, 0, 0};
	//long double distance[3] = {x[body2] - x[body1], y[body2] - y[body1], z[body2] - z[body1]};
	long double distance = sqrt(pow(x[body2] - x[body1],2) + pow(y[body2] - y[body1],2) + pow(z[body2] - z[body1],2));
	long double unit_vector = distance/fabs(distance);	


	force[0] = -G*((mass[body1]*mass[body2]/pow(fabs(distance), 2)))*unit_vector;
	force[1] = -G*((mass[body1]*mass[body2]/pow(fabs(distance), 2)))*unit_vector;
	force[2] = -G*((mass[body1]*mass[body2]/pow(fabs(distance), 2)))*unit_vector;
	...
	long double new_force[3] = {0, 0, 0};
	// long double new_distance[3] = {new_x[body2] - x[body1], new_y[body2] - y[body1], new_z[body2] - z[body1]};
	long double new_distance = sqrt(pow(new_x[body2] - x[body1],2) + pow(new_y[body2] - y[body1],2) + pow(new_z[body2] - z[body1],2));
	long double new_unit_vector = new_distance/fabs(new_distance);
	
	new_force[0] = -G*((mass[body1]*mass[body2]/pow(new_distance, 2)))*new_unit_vector;
	new_force[1] = -G*((mass[body1]*mass[body2]/pow(new_distance, 2)))*new_unit_vector;
	new_force[2] = -G*((mass[body1]*mass[body2]/pow(new_distance, 2)))*new_unit_vector;
